...
let counterOfSliderTwo = 0;

//to reset slides in every change with display none
function resetSlideTwo () {
    slidesTwoArr.forEach( (img) => {img.style.display = 'none';} );
}
...
function slideLeftTwo () {
    resetSlideTwo();
    // it start from last index, min-1 to make the prev index is block after reseting them
...
function slideRightTwo () {
    resetSlideTwo();
    // it start from zero, plus+1 to make the next index is block after reseting them
    slidesTwoArr[counterOfSliderTwo + 1].style.display = 'block';
    counterOfSliderTwo ++;
}
//end slider 2
...
//arrow rigth increase the counter to increase the translateX
slider3Right.addEventListener('click', function(){
    console.log(counterOfSlider3);
    //if we on first-clone we return ( this happen when we click quickly many times as we will be on the index[0])
...
const current4 = document.querySelector('#slider4 .current');

    // current4.classList.remove('current'); // it's right and when make it inside if() right also
...
thumbnailArr5.forEach( (img) => {
    img.addEventListener('click', (e) => {
        ...
        mainImgIn5.src = img.src;
        //to remove class selected from all imgs of thumbnalis
        thumbnailArr5.forEach( (img) => img.classList.remove('selected') );
        //to add the class selected for the clicked thumbnail
        img.classList.add('selected');
        // to add class fade-in to current img 
        mainImgIn5.classList.add('fade-in');
        // to remove the calss after finishing the animation every 400ms to can use it again with the same effect    
...
pullet[0].classList.add('active');

// make main function to change the content & move the classes[disabled on arrows, active on pullet current on imgs] depends on current number 
function main () {
    //to change the content depends on current slide
    slider6Content.textContent = `this slider is ${currentIn6}`;

    // add/remove active from the pullets 
    pullet.forEach( (ele) => ele.classList.remove('active') );
    pullet[currentIn6 - 1].classList.add('active');

    // add/remove current class from the img slider
    imgs6Arr.forEach( (img) => img.classList.remove('current') );
    imgs6Arr[currentIn6 - 1].classList.add('current');

    // add/remove disabled class form arrows
